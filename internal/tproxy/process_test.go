...
func TestStartResolvesRelativeBinaryPathAgainstCallerCwd(t *testing.T) {
	...
	p := newChildProcess("bin/fake-binary", nil, "127.0.0.1:1", "regression", false)
...
// avoid ever exposing euid 0 to a dropPrivileges child in the first place:
// it can't exec the real engine, so it has the fake binary report its own
// uid instead of trusting SysProcAttr silently having no effect.
func TestStartDropsPrivilegesForDropPrivilegesChild(t *testing.T) {
	...
	uidFile := filepath.Join(root, "uid")
	script := fmt.Sprintf("#!/bin/sh\nid -u > %s\n", uidFile)
	if err := os.WriteFile("bin/fake-binary", []byte(script), 0o755); err != nil {
		t.Fatalf("write fake binary: %v", err)
	}

	p := newChildProcess("bin/fake-binary", nil, "127.0.0.1:1", "regression", true)
	if err := p.Start(); err != nil {
	...
	for time.Now().Before(deadline) {
		if got, err = os.ReadFile(uidFile); err == nil {
			break
		}
		time.Sleep(10 * time.Millisecond)
	}
	if err != nil {
		t.Fatalf("read %s: %v (result: %s)", uidFile, err, p.GetResult())
	}
	gotUID := strings.TrimSpace(string(got))
	if gotUID == "0" {
		t.Fatal("child ran as uid 0 -- dropPrivileges had no effect")
	}
