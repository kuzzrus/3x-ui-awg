...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/kuzzrus/3x-ui-awg/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/kuzzrus/3x-ui-awg/main/update.sh)
    if [[ $? == 0 ]]; then
...
update_dev() {
    ...
    XUI_UPDATE_TAG="dev-latest" bash <(curl -Ls https://raw.githubusercontent.com/kuzzrus/3x-ui-awg/main/update.sh)
    if [[ $? == 0 ]]; then
...
installed_script_url() {
    ...
    if [[ "$ver" =~ ^[0-9]+\.[0-9]+\.[0-9]+(-awg\.[0-9]+)?$ ]] && curl -fsIL -o /dev/null "https://raw.githubusercontent.com/kuzzrus/3x-ui-awg/v${ver}/x-ui.sh"; then
        echo "https://raw.githubusercontent.com/kuzzrus/3x-ui-awg/v${ver}/x-ui.sh"
    else
        echo -e "${yellow}No x-ui.sh published for the installed version (${ver:-unknown}), using main${plain}" >&2
        echo "https://raw.githubusercontent.com/kuzzrus/3x-ui-awg/main/x-ui.sh"
    fi
}
