...
_openapi:
  ...
  toc:
    ...
    - depth: 2
      title: Same reload as POST restartXrayService, but responds before the restart
        runs and reports the outcome through GET getRestartStatus. Used by the
        panel’s own dashboard; other callers should keep using
        restartXrayService.
      url: '#same-reload-as-post-restartxrayservice-but-responds-before-the-restart-runs-and-reports-the-outcome-through-get-getrestartstatus-used-by-the-panels-own-dashboard-other-callers-should-keep-using-restartxrayservice'
    - depth: 2
      title: Report the outcome of the most recently started dashboard restart (see
        POST restartXrayServiceAsync). Compare the returned runId against the
        one restartXrayServiceAsync returned to tell this run apart from a stale
        result.
      url: '#report-the-outcome-of-the-most-recently-started-dashboard-restart-see-post-restartxrayserviceasync-compare-the-returned-runid-against-the-one-restartxrayserviceasync-returned-to-tell-this-run-apart-from-a-stale-result'
    - depth: 2
    ...
    - depth: 2
      title: Report the host’s actual unattended-upgrades configuration (read from
        /etc/apt/apt.conf.d/ directly, never a cached flag) plus the outcome of
        the most recent runNow.
      url: '#report-the-hosts-actual-unattended-upgrades-configuration-read-from-etcaptaptconfd-directly-never-a-cached-flag-plus-the-outcome-of-the-most-recent-runnow'
    - depth: 2
      title: Install the unattended-upgrades OS package via apt-get if it is not
        already present. Does not change any setting -- see configure.
      url: '#install-the-unattended-upgrades-os-package-via-apt-get-if-it-is-not-already-present-does-not-change-any-setting----see-configure'
    - depth: 2
      title: Write this panel’s own apt.conf.d drop-in (update scope, auto-reboot) and
        turn the periodic timer on.
      url: '#write-this-panels-own-aptconfd-drop-in-update-scope-auto-reboot-and-turn-the-periodic-timer-on'
    - depth: 2
      title: Turn off the periodic timer and remove this panel’s own drop-in. Does not
        remove the unattended-upgrades package itself.
      url: '#turn-off-the-periodic-timer-and-remove-this-panels-own-drop-in-does-not-remove-the-unattended-upgrades-package-itself'
    - depth: 2
      title: Run unattended-upgrade immediately in the background and return a runId.
        Poll GET status (lastRun) for the outcome, the same shape
        updatePanel/getUpdateStatus use.
      url: '#run-unattended-upgrade-immediately-in-the-background-and-return-a-runid-poll-get-status-lastrun-for-the-outcome-the-same-shape-updatepanelgetupdatestatus-use'
    - depth: 2
    ...
    - depth: 2
      title: Send a real QUICv1 Initial ClientHello to host:443 and turn the server's
        real Initial reply into an AmneziaWG CPS chain, ready to drop into an
        inbound's I1-I5 field. Fails if the host does not respond on UDP/443
        within a few seconds.
      url: '#send-a-real-quicv1-initial-clienthello-to-host443-and-turn-the-servers-real-initial-reply-into-an-amneziawg-cps-chain-ready-to-drop-into-an-inbounds-i1-i5-field-fails-if-the-host-does-not-respond-on-udp443-within-a-few-seconds'
    - depth: 2
  ...
  structuredData:
    headings:
      ...
      - content: Same reload as POST restartXrayService, but responds before the restart
          runs and reports the outcome through GET getRestartStatus. Used by the
          panel’s own dashboard; other callers should keep using
          restartXrayService.
        id: same-reload-as-post-restartxrayservice-but-responds-before-the-restart-runs-and-reports-the-outcome-through-get-getrestartstatus-used-by-the-panels-own-dashboard-other-callers-should-keep-using-restartxrayservice
      - content: Report the outcome of the most recently started dashboard restart (see
          POST restartXrayServiceAsync). Compare the returned runId against the
          one restartXrayServiceAsync returned to tell this run apart from a
          stale result.
        id: report-the-outcome-of-the-most-recently-started-dashboard-restart-see-post-restartxrayserviceasync-compare-the-returned-runid-against-the-one-restartxrayserviceasync-returned-to-tell-this-run-apart-from-a-stale-result
      ...
      - content: Report the host’s actual unattended-upgrades configuration (read from
          /etc/apt/apt.conf.d/ directly, never a cached flag) plus the outcome
          of the most recent runNow.
        id: report-the-hosts-actual-unattended-upgrades-configuration-read-from-etcaptaptconfd-directly-never-a-cached-flag-plus-the-outcome-of-the-most-recent-runnow
      - content: Install the unattended-upgrades OS package via apt-get if it is not
          already present. Does not change any setting -- see configure.
        id: install-the-unattended-upgrades-os-package-via-apt-get-if-it-is-not-already-present-does-not-change-any-setting----see-configure
      - content: Write this panel’s own apt.conf.d drop-in (update scope, auto-reboot)
          and turn the periodic timer on.
        id: write-this-panels-own-aptconfd-drop-in-update-scope-auto-reboot-and-turn-the-periodic-timer-on
      - content: Turn off the periodic timer and remove this panel’s own drop-in. Does
          not remove the unattended-upgrades package itself.
        id: turn-off-the-periodic-timer-and-remove-this-panels-own-drop-in-does-not-remove-the-unattended-upgrades-package-itself
      - content: Run unattended-upgrade immediately in the background and return a
          runId. Poll GET status (lastRun) for the outcome, the same shape
          updatePanel/getUpdateStatus use.
        id: run-unattended-upgrade-immediately-in-the-background-and-return-a-runid-poll-get-status-lastrun-for-the-outcome-the-same-shape-updatepanelgetupdatestatus-use
      ...
      - content: Send a real QUICv1 Initial ClientHello to host:443 and turn the
          server's real Initial reply into an AmneziaWG CPS chain, ready to drop
          into an inbound's I1-I5 field. Fails if the host does not respond on
          UDP/443 within a few seconds.
        id: send-a-real-quicv1-initial-clienthello-to-host443-and-turn-the-servers-real-initial-reply-into-an-amneziawg-cps-chain-ready-to-drop-into-an-inbounds-i1-i5-field-fails-if-the-host-does-not-respond-on-udp443-within-a-few-seconds
