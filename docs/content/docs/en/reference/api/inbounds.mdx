...
_openapi:
  ...
  toc:
    ...
    - depth: 2
      title: "Live, read-only snapshot of a running AmneziaWG inbound: interface
        up/down, listen port, and each configured client's handshake/traffic
        state. Gathering it can never itself change anything; an inbound that
        simply isn't running right now (disabled, no enabled clients, or the
        reconcile loop hasn't caught up yet) comes back with running=false
        rather than an error."
      url: '#live-read-only-snapshot-of-a-running-amneziawg-inbound-interface-updown-listen-port-and-each-configured-clients-handshaketraffic-state-gathering-it-can-never-itself-change-anything-an-inbound-that-simply-isnt-running-right-now-disabled-no-enabled-clients-or-the-reconcile-loop-hasnt-caught-up-yet-comes-back-with-runningfalse-rather-than-an-error'
  structuredData:
    headings:
      ...
      - content: "Live, read-only snapshot of a running AmneziaWG inbound: interface
          up/down, listen port, and each configured client's handshake/traffic
          state. Gathering it can never itself change anything; an inbound that
          simply isn't running right now (disabled, no enabled clients, or the
          reconcile loop hasn't caught up yet) comes back with running=false
          rather than an error."
        id: live-read-only-snapshot-of-a-running-amneziawg-inbound-interface-updown-listen-port-and-each-configured-clients-handshaketraffic-state-gathering-it-can-never-itself-change-anything-an-inbound-that-simply-isnt-running-right-now-disabled-no-enabled-clients-or-the-reconcile-loop-hasnt-caught-up-yet-comes-back-with-runningfalse-rather-than-an-error
    contents: []
---
