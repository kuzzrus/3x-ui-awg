  ...
  toc:
    - depth: 2
      title: Return the Xray config template (JSON string), available inbound tags,
        client reverse tags, and the configured outbound test/speed-test URLs in
        one response.
      url: '#return-the-xray-config-template-json-string-available-inbound-tags-client-reverse-tags-and-the-configured-outbound-testspeed-test-urls-in-one-response'
    - depth: 2
    ...
    - depth: 2
      title: Save the Xray JSON config template and optionally the outbound
        test/speed-test URLs. All are sent as form fields.
      url: '#save-the-xray-json-config-template-and-optionally-the-outbound-testspeed-test-urls-all-are-sent-as-form-fields'
    - depth: 2
    ...
    - depth: 2
      title: Manage the panel-hosted Tor sidecar. The action parameter selects the
        operation.
      url: '#manage-the-panel-hosted-tor-sidecar-the-action-parameter-selects-the-operation'
    - depth: 2
      title: Manage the panel-hosted Psiphon sidecar. The action parameter selects the
        operation.
      url: '#manage-the-panel-hosted-psiphon-sidecar-the-action-parameter-selects-the-operation'
    - depth: 2
      title: Replace the admin-supplied psiphon.config. The panel does not and cannot
        generate working Psiphon credentials -- this must come from the admin.
      url: '#replace-the-admin-supplied-psiphonconfig-the-panel-does-not-and-cannot-generate-working-psiphon-credentials----this-must-come-from-the-admin'
    - depth: 2
      title: List ISO 3166-1 codes (states plus common territories) for the
        egress-region picker (not a Psiphon-curated subset -- see the
        psiphon/:action region action to actually apply and verify one).
      url: '#list-iso-3166-1-codes-states-plus-common-territories-for-the-egress-region-picker-not-a-psiphon-curated-subset----see-the-psiphonaction-region-action-to-actually-apply-and-verify-one'
    - depth: 2
      title: Manage the panel-hosted WARP-via-wireproxy sidecar
        (github.com/kuzzrus/WARP_WireProxy_Manager, vendored and pinned). The
        action parameter selects the operation.
      url: '#manage-the-panel-hosted-warp-via-wireproxy-sidecar-githubcomkuzzruswarp_wireproxy_manager-vendored-and-pinned-the-action-parameter-selects-the-operation'
    - depth: 2
    ...
    - depth: 2
      title: Manage the built-in reverse proxy that Xray's REALITY fallback target
        points at. The action parameter selects the operation.
      url: '#manage-the-built-in-reverse-proxy-that-xrays-reality-fallback-target-points-at-the-action-parameter-selects-the-operation'
    - depth: 2
      title: Replace the reverse proxy decoy with an uploaded static site. The zip
        must contain an index.html, either at its root or inside a single
        wrapping folder.
      url: '#replace-the-reverse-proxy-decoy-with-an-uploaded-static-site-the-zip-must-contain-an-indexhtml-either-at-its-root-or-inside-a-single-wrapping-folder'
    - depth: 2
      title: Manage the AdGuard Home instance the reverse proxy can serve as its
        decoy. Unlike the template decoys this is a real service, so the domain
        answers with a working admin UI and DNS-over-HTTPS.
      url: '#manage-the-adguard-home-instance-the-reverse-proxy-can-serve-as-its-decoy-unlike-the-template-decoys-this-is-a-real-service-so-the-domain-answers-with-a-working-admin-ui-and-dns-over-https'
    - depth: 2
    ...
    - depth: 2
      title: Test a batch of outbounds (max 50) through one shared temp xray instance.
        ...
        delay, HTTP status, a connect/TLS/TTFB timing breakdown, or (mode
        "speed") download/upload Mbit/s.
      url: '#test-a-batch-of-outbounds-max-50-through-one-shared-temp-xray-instance-returns-an-array-of-results-in-input-order-each-with-the-outbound-tag-delay-http-status-a-connecttlsttfb-timing-breakdown-or-mode-speed-downloadupload-mbits'
    - depth: 2
  ...
  structuredData:
    headings:
      - content: Return the Xray config template (JSON string), available inbound tags,
          client reverse tags, and the configured outbound test/speed-test URLs
          in one response.
        id: return-the-xray-config-template-json-string-available-inbound-tags-client-reverse-tags-and-the-configured-outbound-testspeed-test-urls-in-one-response
      ...
      - content: Save the Xray JSON config template and optionally the outbound
          test/speed-test URLs. All are sent as form fields.
        id: save-the-xray-json-config-template-and-optionally-the-outbound-testspeed-test-urls-all-are-sent-as-form-fields
      ...
      - content: Manage the panel-hosted Tor sidecar. The action parameter selects the
          operation.
        id: manage-the-panel-hosted-tor-sidecar-the-action-parameter-selects-the-operation
      - content: Manage the panel-hosted Psiphon sidecar. The action parameter selects
          the operation.
        id: manage-the-panel-hosted-psiphon-sidecar-the-action-parameter-selects-the-operation
      - content: Replace the admin-supplied psiphon.config. The panel does not and
          cannot generate working Psiphon credentials -- this must come from the
          admin.
        id: replace-the-admin-supplied-psiphonconfig-the-panel-does-not-and-cannot-generate-working-psiphon-credentials----this-must-come-from-the-admin
      - content: List ISO 3166-1 codes (states plus common territories) for the
          egress-region picker (not a Psiphon-curated subset -- see the
          psiphon/:action region action to actually apply and verify one).
        id: list-iso-3166-1-codes-states-plus-common-territories-for-the-egress-region-picker-not-a-psiphon-curated-subset----see-the-psiphonaction-region-action-to-actually-apply-and-verify-one
      - content: Manage the panel-hosted WARP-via-wireproxy sidecar
          (github.com/kuzzrus/WARP_WireProxy_Manager, vendored and pinned). The
          action parameter selects the operation.
        id: manage-the-panel-hosted-warp-via-wireproxy-sidecar-githubcomkuzzruswarp_wireproxy_manager-vendored-and-pinned-the-action-parameter-selects-the-operation
      ...
      - content: Manage the built-in reverse proxy that Xray's REALITY fallback target
          points at. The action parameter selects the operation.
        id: manage-the-built-in-reverse-proxy-that-xrays-reality-fallback-target-points-at-the-action-parameter-selects-the-operation
      - content: Replace the reverse proxy decoy with an uploaded static site. The zip
          must contain an index.html, either at its root or inside a single
          wrapping folder.
        id: replace-the-reverse-proxy-decoy-with-an-uploaded-static-site-the-zip-must-contain-an-indexhtml-either-at-its-root-or-inside-a-single-wrapping-folder
      - content: Manage the AdGuard Home instance the reverse proxy can serve as its
          decoy. Unlike the template decoys this is a real service, so the
          domain answers with a working admin UI and DNS-over-HTTPS.
        id: manage-the-adguard-home-instance-the-reverse-proxy-can-serve-as-its-decoy-unlike-the-template-decoys-this-is-a-real-service-so-the-domain-answers-with-a-working-admin-ui-and-dns-over-https
      ...
      - content: Test a batch of outbounds (max 50) through one shared temp xray
          ...
          outbound tag, delay, HTTP status, a connect/TLS/TTFB timing breakdown,
          or (mode "speed") download/upload Mbit/s.
        id: test-a-batch-of-outbounds-max-50-through-one-shared-temp-xray-instance-returns-an-array-of-results-in-input-order-each-with-the-outbound-tag-delay-http-status-a-connecttlsttfb-timing-breakdown-or-mode-speed-downloadupload-mbits
