...
Каждый файл релиза публикуется вместе с суммой `.sha256`. И `install.sh`, и апдейтер проверяют архив по этой сумме и прерывают установку при несовпадении.
...
| `NODE_TOKEN_ENCRYPTION` | Шифрование токенов узлов при хранении: `off`, `migration` или `required` (без префикса `XUI_`) | `off` |
| `XUI_NODE_TOKEN_KEY_FILE` | JSON-связка ключей (права `0600`) с активным id ключа и его base64 32-байтными ключами | `/etc/x-ui/node_token_key.json` |
| `XUI_NODE_TOKEN_KEY` | Один base64 32-байтный ключ, используется только если файл ключей загрузить не удалось | — |
...
- [3X-UI Manager](https://github.com/yukh975/3X-UI-Manager) (**MIT**) — нативный Android-клиент для 3x-ui: дашборд, инбаунды, клиенты с шарингом по QR, узлы и управление несколькими панелями. Доступен на F-Droid.
