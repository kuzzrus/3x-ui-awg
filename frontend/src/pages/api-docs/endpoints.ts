...

  {
    ...
    endpoints: [
      ...
      },
      {
        ...
        summary:
          "Live, read-only snapshot of a running AmneziaWG inbound: interface up/down, listen port, and each configured client's handshake/traffic state. Gathering it can never itself change anything; an inbound that simply isn't running right now (disabled, no enabled clients, or the reconcile loop hasn't caught up yet) comes back with running=false rather than an error.",
        params: [{ name: 'id', in: 'path', type: 'number', desc: 'AmneziaWG inbound ID.' }],
        response:
...

  {
    ...
    endpoints: [
      ...
      },
      {
        ...
        summary:
          "Send a real QUICv1 Initial ClientHello to host:443 and turn the server's real Initial reply into an AmneziaWG CPS chain, ready to drop into an inbound's I1-I5 field. Fails if the host does not respond on UDP/443 within a few seconds.",
        params: [
          {
            name: 'host',
            in: 'body (form)',
            type: 'string',
            desc: 'Target host to capture a real QUIC exchange from, e.g. www.google.com. Resolved and rejected if it lands on a private/loopback/link-local address.',
          },
        ],
    ...
    endpoints: [
      {
        ...
        path: '/panel/api/xray/',
        summary:
          'Return the Xray config template (JSON string), available inbound tags, client reverse tags, and the configured outbound test/speed-test URLs in one response.',
        response:
      ...
      },
      {
        ...
        summary:
          'Save the Xray JSON config template and optionally the outbound test/speed-test URLs. All are sent as form fields.',
        params: [
          {
            name: 'xraySetting',
            in: 'body (form)',
          ...
          {
            name: 'speedDownloadUrl',
            in: 'body (form)',
            type: 'string',
            desc: 'URL used for speed-test downloads. Defaults to https://proof.ovh.net/files/1Gb.dat.',
          },
          {
            name: 'speedUploadUrl',
            in: 'body (form)',
            type: 'string',
            desc: 'URL used for speed-test uploads. Defaults to https://speed.cloudflare.com/__up.',
          },
        ],
      },
      {
        ...
        params: [
          {
            ...
            in: 'path',
            type: 'string',
            desc: 'status — return {available, running, port, lastLog}. start — launch the managed tor daemon. stop — stop it. install — install the tor package via the host package manager. uninstall — stop the daemon and remove the package. ip — dial out through the SOCKS port and return {ip, isTor} from check.torproject.org. newIdentity — SIGNAL NEWNYM (request a new circuit).',
          },
        ],
      },
      {
      ...
      },
      {
        ...
        summary:
          "Manage the built-in reverse proxy that Xray's REALITY fallback target points at. The action parameter selects the operation.",
        params: [
          {
            name: 'action',
            in: 'path',
            type: 'string',
            desc: 'status — return {running, port, templates, decoyUploaded}. start — bring the reverse-proxy listener up and remember the choice across restarts. stop — shut it down, also remembered. removeDecoy — delete the uploaded decoy site. Port, certificate and decoy settings are persisted separately and only take effect after a panel restart.',
          },
        ],
      },
      {
        ...
        summary:
          'Replace the reverse proxy decoy with an uploaded static site. The zip must contain an index.html, either at its root or inside a single wrapping folder.',
        params: [
          {
            name: 'site',
            in: 'body (multipart)',
            type: 'file',
            desc: 'Zip archive of the site. Capped at 64 MiB and 2000 entries; entries with absolute or traversing paths are rejected.',
          },
        ],
      ...
      },
      {
        ...
        summary:
          'Test a batch of outbounds (max 50) through one shared temp xray instance. Returns an array of results in input order, each with the outbound tag, delay, HTTP status, a connect/TLS/TTFB timing breakdown, or (mode "speed") download/upload Mbit/s.',
        params: [
          {
            name: 'outbounds',
            in: 'body (form)',
          ...
          },
          {
            ...
            in: 'body (form)',
            type: 'string',
            desc: '"tcp" for fast dial-only probes (UDP-transport outbounds are still probed over HTTP), "real" for real-delay probes whose delay is the full request time including tunnel establishment, "speed" for a timed down+up throughput measurement per outbound (always run at concurrency 1, regardless of batch size — several outbounds tested at once would contend for the same uplink/downlink). Default/empty routes an HTTP request through each outbound and reports the warm per-request round-trip.',
          },
          {
            name: 'speedDownloadUrl',
            in: 'body (form)',
