...
describe('AmneziaWG .conf emitters agree on MTU', () => {
  ...
  it('always emits an MTU, even when the inbound has none set', () => {
    ...
    // mtuPathCeiling(1440) - 27: without this the client stays on its own
    // 1420 default and fragments every full-size packet it sends.
    expect(mtuLine(link)).toBe('MTU = 1413');
    expect(mtuLine(download)).toBe('MTU = 1413');
  });
